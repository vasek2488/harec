  ...
  persist(
    (set, get) => ({
      ...
      saveExamResult: (examResult: ExamResult) => {
        set((state) => {
          if (
            state.savedExamResultIds.includes(examResult.id) ||
            state.examHistory.some((result) => result.id === examResult.id)
          ) {
            return {
              savedExamResultIds: state.savedExamResultIds.includes(examResult.id)
                ? state.savedExamResultIds
                : [...state.savedExamResultIds, examResult.id],
            };
          }
