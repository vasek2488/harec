...
describe('Progress Store', () => {
  ...
  describe('saveExamResult', () => {
    ...
    it('should ignore duplicate saves for the same exam id', () => {
      const store = useProgressStore.getState();
      const examResult: ExamResult = {
        id: 'exam-1',
        startedAt: new Date().toISOString(),
      ...
      expect(useProgressStore.getState().examHistory).toHaveLength(1);
      expect(useProgressStore.getState().savedExamResultIds).toEqual(['exam-1']);

      const stat = useProgressStore.getState().getQuestionStat('q-1');
      expect(stat?.timesAnswered).toBe(1);
    });
  });
