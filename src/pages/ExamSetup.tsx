...
export function ExamSetup() {
  ...
  const questionBankEmpty = allQuestions.length === 0;
  const integrityError =
    contentError ??
    (questionBankEmpty
      ? "Chyba obsahu: banka otázek je prázdná. Test nelze spustit."
      : null);

  const handleStart = () => {
    if (questionBankEmpty) return;

    const session = generateExamSession(config, allQuestions);
    if (session.questionIds.length === 0) {
      setContentError(
        "Chyba obsahu: nepodařilo se sestavit test z dostupné banky otázek."
      );
      return;
    }

    startSession("exam", session.questionIds);
