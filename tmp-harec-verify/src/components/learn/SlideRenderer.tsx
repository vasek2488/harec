...
function FillBlankSlide({
  ...
  const shouldShuffleChoices = getConfig().examRules.shuffleChoices;
  const [choices] = useState(() =>
    shouldShuffleChoices
      ? shuffleArray([blank, ...distractors])
      : [blank, ...distractors]
  );
...
function SelectAnswerSlide({
  ...
  const [displayChoices] = useState(() =>
    shouldShuffleChoices ? shuffleArray(choices) : [...choices]
  );
...
function HighlightSlide({
  ...
  return (
    <div className="space-y-4">
      ...
      {answered && submittedCorrect !== null && (
        <Feedback correct={submittedCorrect} explanation={explanation} />
      )}
    </div>
  );
}
