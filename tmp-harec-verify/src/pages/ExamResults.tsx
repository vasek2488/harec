...
export function ExamResults() {
  ...
  const { answers, questionIds, sessionStartedAt, sessionEndedAt, sessionType } =
    sessionStore;
  const allQuestions = getQuestions();
  const config = getConfig();
  const sessionId = `exam:${sessionStartedAt ?? "unknown"}:${questionIds.join(",")}`;

  const score = calculateScore(answers, config.examRules);
  const partScores = calculatePartScores(answers, allQuestions, config);

  // Save exam result once on mount
  useEffect(() => {
    if (answers.length === 0 || sessionType !== "exam")
      return;
